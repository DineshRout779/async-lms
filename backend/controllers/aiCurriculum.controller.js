...
exports.updateLesson = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, explanation, example, activity, interview_questions, lesson_type, duration_mins, video_url, exercise_data } = req.body;
    const sets = [];
    const vals = [];
    let i = 1;
    ...
    if (video_url !== undefined)           add('video_url', video_url);
     if (exercise_data !== undefined) {
      add('exercise_data', exercise_data);
    }
    if (!sets.length) return res.status(400).json({ success: false, message: 'Nothing to update' });
    vals.push(id);
