...
function FullPreviewModal({ data, sections, onClose }: { data: ResumeData; sections: Section[]; onClose: () => void }) {
  return (
...
export default function ResumeBuilder() {
  ...
  const handleOptimizeJD = async () => {
    ...
    setJdLoading(true);
    try {
      const res = await apiClient.post('/assistant/resume/optimize', { resumeData, jobDescription: jdText });
      const result = res.data.data as ATSResult;
      setAtsResult(result);
      if (result.optimizedSummary) {
        setResumeData((prev) => prev ? { ...prev, summary: result.optimizedSummary! } : prev);
      }
    ...
    <div className='flex flex-col h-full bg-[#f7f8fc]'>
      ...
      <div className='flex items-center justify-between px-6 py-4 bg-white border-b border-slate-100 shrink-0'>
        <div>
          ...
          <p className='text-xs text-slate-400 mt-0.5'>ATS-optimized resume builder with AI assistance</p>
        </div>
        <div className='flex items-center gap-2'>
          <ActionBtn icon={<Maximize2 size={14} />} label='Full Preview' onClick={() => setShowFullPreview(true)} disabled={!resumeData} />
          <ActionBtn
            icon={<CheckSquare size={14} />}
            label='Check ATS Score'
            onClick={() => resumeData ? setAtsResult(calcLocalATS(resumeData)) : toast.error('Generate resume first.')}
            disabled={!resumeData}
          />
          <ActionBtn icon={<Download size={14} />} label='Download PDF' onClick={handleExportPDF} disabled={!resumeData} />
          <button
      ...
      <div className='flex flex-1 overflow-hidden'>
        ...
        <aside className='w-72 shrink-0 bg-white border-r border-slate-100 flex flex-col overflow-y-auto'>
          ...
          <div className='p-4 border-b border-slate-100'>
            <div className='flex items-center justify-between mb-3'>
              <span className='text-xs font-semibold text-slate-700'>ATS Score</span>
              <div className='flex items-baseline gap-1'>
                <span className='text-2xl font-black text-[#333d7c]'>{localATS.atsScore}</span>
                <span className='text-xs text-slate-400'>/100</span>
              </div>
            </div>
            <ScoreBar label='Keyword match' value={localATS.keywordMatch} color='bg-blue-400' />
            <ScoreBar label='Content quality' value={localATS.contentQuality} color='bg-emerald-400' />
            <ScoreBar label='Formatting' value={localATS.formatting} color='bg-green-400' />

            {atsResult?.suggestions?.length || atsResult?.missingKeywords?.length ? (
          ...
          <div className='p-4 border-b border-slate-100'>
            ...
            <label className='text-[11px] text-slate-500 mb-1 block'>Job Description</label>
            <textarea
            ...
            >
              {jdLoading ? <Loader2 size={13} className='animate-spin' /> : <Wand2 size={13} />}
              Optimize with JD
            </button>
          </div>
          ...
          <div className='p-4 flex-1'>
            <div className='flex items-center justify-between mb-2'>
              <span className='text-xs font-semibold text-slate-700'>Sections</span>
        ...
        <main className='flex-1 overflow-y-auto p-8'>
          {resumeData ? (
            <div className='max-w-3xl mx-auto bg-white shadow-sm rounded-lg p-10 min-h-225' ref={previewRef}>
              <ResumeDocument data={resumeData} sections={sections} onUpdate={setResumeData} />
            </div>
          ) : (
            <div className='max-w-3xl mx-auto h-full min-h-[600px] flex flex-col items-center justify-center'>
              ...
              <h2 className='text-xl font-bold text-[#1e2653] mb-2'>Start building your resume</h2>
              <p className='text-sm text-slate-400 text-center max-w-sm mb-8'>
                Your profile, course completions, and earned badges will be auto-fetched. Add a few extra details and let AI do the rest.
              </p>
              <div className='grid grid-cols-3 gap-4 mb-8'>
                {[
                  { icon: <User size={18} className='text-[#333d7c]' />, text: 'Profile auto-fetched' },
                  { icon: <GraduationCap size={18} className='text-[#333d7c]' />, text: 'Courses & skills detected' },
                  { icon: <Award size={18} className='text-[#333d7c]' />, text: 'Badges included' },
                ].map((item, i) => (
                  <div key={i} className='bg-white rounded-xl p-4 shadow-sm flex flex-col items-center gap-2 text-center'>
                    <div className='w-9 h-9 rounded-full bg-[#eef0fb] flex items-center justify-center'>{item.icon}</div>
      ...
      {showFullPreview && resumeData && (
        <FullPreviewModal data={resumeData} sections={sections} onClose={() => setShowFullPreview(false)} />
      )}
    </div>
  );
}
