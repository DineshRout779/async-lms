    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
      ...
      <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden'>
        ...
        <div className='flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0'>
          <div>
            <h2 className='text-base font-bold text-slate-800'>Unit Quiz</h2>
            <p className='text-[12px] text-slate-400 mt-0.5 truncate max-w-sm'>{topicTitle}</p>
          </div>
        ...
        <div className='overflow-y-auto flex-1 px-6 py-4 space-y-3'>
          {questions.length === 0 && !addingNew && (
            <div className='py-12 text-center'>
              <p className='text-[13px] text-slate-400'>No questions yet.</p>
              {canEdit && (
              ...
              {editingIdx === idx ? (
                ...
                <div className='p-4 bg-indigo-50/30'>
                  ...
                  <div className='flex justify-end gap-2 mt-3'>
                    <button onClick={cancelEdit} className='px-3 py-1.5 text-[12px] font-semibold text-slate-500 hover:text-slate-700 transition-colors'>
                      Cancel
                    </button>
                    <button
                      onClick={() => handleSaveEdit(idx, editDrafts[idx] ?? q)}
                      disabled={saving}
                      className='flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors'
                    >
                ...
                <div className='p-4'>
                  <div className='flex items-start gap-2 mb-3'>
                    ...
                    <p className='text-[13px] font-semibold text-slate-800 flex-1'>{q.question}</p>
                    {canEdit && (
                      <div className='flex items-center gap-1 shrink-0'>
                        <button
                          onClick={() => startEdit(idx)}
                          className='px-2 py-1 text-[11px] font-semibold text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors'
                        >
                          Edit
                        </button>
                        <button
                  ...
                  {q.explanation && (
                    <p className='mt-2.5 text-[11px] text-blue-600 bg-blue-50 rounded-lg px-3 py-2 border border-blue-100'>
                      {q.explanation}
                    </p>
                  )}
                </div>
          ...
          {addingNew && (
            <div className='border-2 border-indigo-200 rounded-xl p-4 bg-indigo-50/20'>
              ...
              <div className='flex justify-end gap-2 mt-3'>
                <button onClick={() => { setAddingNew(false); setNewQ(emptyQuestion()); }} className='px-3 py-1.5 text-[12px] font-semibold text-slate-500 hover:text-slate-700 transition-colors'>
                  Cancel
                </button>
                <button
        ...
        {canEdit && !addingNew && (
          <div className='border-t border-slate-100 px-6 py-3 shrink-0'>
            <button
              ...
              className='flex items-center gap-2 px-4 py-2 text-[13px] font-semibold text-indigo-600 border-2 border-dashed border-indigo-200 rounded-lg hover:bg-indigo-50 transition-colors w-full justify-center'
            >
