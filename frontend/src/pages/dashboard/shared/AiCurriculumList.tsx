...
function CourseCard({
  ...
  const moduleCount = (course as any).modules_count ?? course.modules?.length ?? 0;
  const topicCount = (course as any).topics_count ?? course.modules?.reduce((acc, m) => acc + (m.topics?.length ?? 0), 0) ?? 0;
